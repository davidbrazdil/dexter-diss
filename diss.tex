...
Returns the taint tag of given object. Returns 0 when \verb$obj == null$.
...
The following snippets of Dalvik assembly will always show the original instruction in bold typeface and the inserted instructions in regular font. They will also use the \verb$taint-get$ and \verb$taint-set$ pseudo-instructions (explained in section ~\ref{section:Code_Pseudoinstructions}) as a shorthand for calling the \verb$get$ and \verb$set$ methods of the taint-storing hash map for objects. Their arguments are:
\centerbox{
...
In previous examples, inserted instructions only addressed taint registers. When arguments are reference-based, methods of the hash map are called and these arguments are passed to it. However, they might not be available after the original instruction any more, because they can be overwritten by the instruction's result. 

This problem is illustrated by the following rule, in which the original instruction retrieves a reference stored in an instance class field of some object. Since the object might have been assigned taint after the reference was already stored in its field, taint of the object must be propagated to the result. In case \verb$rValue$ and \verb$rObject$ map to the same register, taint of the object must be retrieved before the original instruction. And because the value of the field is not known before, calls to \verb$taint-set$ must be added after the instruction.
