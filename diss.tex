...
It was already explained that the object taint map serves as a runtime function from the set of all object references to integers representing their current taint, and that it is one of the auxiliary classes implemented in Java and linked in. The data structure necessary to implement this function is a regular hash table, but it needs to use weak object references, because storing them in the table does not prevent garbage collection.
...
In its current implementation, the hash map is initalized with 1024 slots and does not support resizing. It exploits temporal locality by moving an entry to the beginning of the linked list whenever it is referenced. Methods are declared as \verb$public static$ so that they can be directly accessed from any place in the application. Data are stored in \verb$private static$ fields.
