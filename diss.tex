...
Dynamic taint analysis relies on the ability to assign attributes, called \emph{taint tags}, to data in the memory, here representing the potential to contain private information. The execution of the program is altered such that tags are assigned to tracked data at the points of their origin, known as \emph{sources}, and inputs of \emph{sinks}, operations capable of sending data away, are monitored to make sure no tainted data leave the system. Between sources and sinks it is made sure that tags propagate correctly as data are moved and combined. 

In the context of Android, sources and sinks are resources like the contacts database and network sockets. Access to these resources needs to be monitored by the taint tracking system, and the semantics of VM instructions extended according to the rules of propagation logic.

The approach adopted by TaintDroid monitors sources and sinks on kernel level and modifies the VM and the file-system to propagate taint. This allows it to track data flow throughout the interpreted code, even between applications. Dexter, limited by the boundaries of the VM, performs the analysis only within a single application by instrumenting its executable code. Resources are accessed through framework methods, and it cannot track taint outside the scope of the methods defined by the application, so-called \emph{internal code}, not even through the framework. This is summarized in Figure~\ref{fig:Introduction_TaintingOverview}. 
