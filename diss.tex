...
The most notable difference is that Dalvik does not distinguish between CPU registers and external memory. Methods are allowed to use up to 65,536 virtual registers, which can be thought of as an equivalent of stack memory. The VM maps some of the registers to the physical registers for efficiency. As we have already seen, most instructions are only capable of addressing the first 16 or 256 registers, and only a limited subset, like the \verb$move$ instructions, can address the rest. Graph colouring is therefore not limited to a fixed number of colours, but each node has its own upper identifier limit.
