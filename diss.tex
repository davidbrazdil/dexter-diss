...
Dynamic taint analysis relies on the ability to assign attributes, called \emph{taint tags}, to data in the memory, here representing the potential to contain private information. The execution of the program is altered such that tags are assigned to data at the points of their origin, known as \emph{sources}, and inputs of \emph{sinks}, operations capable of sending data away, are monitored to make sure no tainted data leave the system. Between sources and sinks it is made sure that tags propagate correctly as data are moved and combined. 

In the context of Android, sources and sinks are represented by various methods provided in the framework, and the semantics of VM instructions need to be extended according to the rules of propagation logic, summarized in Figure~\ref{fig:Introduction_TaintingOverview}. As an example, imagine an application that reads the phone book and sends it to a remote server via HTTP. Methods that can read it are monitored and their results are tagged. Propagation logic ensures that the HTTP request inherits the tag, and passing it to an API call that opens a network socket triggers a warning. 
