...
In both cases, instrumentation should also assign the taint of \verb$this$ to all the parameters, including the reference-based ones, provided the method is not static or a constructor. This is useful especially for propagating taint into callbacks, in situations like GPS updates shown in listing ~\ref{listing:Sink_Location_Listener}. As will be shown shortly, \verb$locationListener$ inherits location taint from \verb$locationManager$ when it is passed to the \verb$requestLocationUpdates$ external method. When its own \verb$onLocationChanged$ method is later called by the service, location taint is correctly assigned to the parameter.
...
Unfortunatelly, the assumptions are generally not safe, and the snippet of code in listing ~\ref{listing:ExternalMethodCalls_LogicHole} shows how tainting could potentially be bypassed. It depends on the external environment containing a class that stores data inside invisible fields and then accesses and leaks these data from untrackable code. From the Android documentation, it seems that sinks currently identified by Dexter do not have this property. Some combination of framework calls could, however, lead to unidentified leakage.

\begin{figure}[h]
	\lstset{
		caption = {Example of a propagation logic hole},
	...
	\begin{lstlisting}
		...
		objA.useX(); 					// use B without passing it
	\end{lstlisting}
\end{figure}
...
\begin{figure}
	\lstset{
		caption = {Example of a contact database query},
...
\begin{figure}
	\lstset{
		caption = {Example of a location-updating callback class},
		label = {listing:Sink_Location_Listener}
	}
	\begin{lstlisting}
