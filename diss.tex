...
In both cases, instrumentation should also assign the taint of \verb$this$ to all the parameters, including the reference-based ones, provided the method is not static or a constructor. This is useful especially for propagating taint into callbacks, in situations like GPS updates shown in listing ~\ref{listing:Source_Location_Listener}. As will be shown shortly, \verb$locationListener$ inherits location taint from \verb$locationManager$ when it is passed to the \verb$requestLocationUpdates$ external method. When its own \verb$onLocationChanged$ method is later called by the service, location taint is correctly assigned to the parameter.
...
The type of the source is given entirely by the URI argument, which is just a textual address. Dexter therefore only needs to insert a series of \verb$if$ statements that compare the first arguement against a list of recognised URIs (see table ~\ref{table:Source_DatabaseQuery_URIs}), and assign the right taint tag to \verb$Cursor$ if a match is found.
...
\begin{table}[h]
	...
	\begin{tabular}{|l|l|}
		\firsthline
		\textbf{Source type} & \textbf{Query URI} \\
		\hline
		contacts             & \verb$content://com.android.contacts$ \\
		text messages        & \verb$content://sms$ \\
		recent calls         & \verb$content://call_log$ \\
		\lasthline
	\end{tabular}
	\end{center}
	\caption{Content query URIs recognized by Dexter}
	\label{table:Source_DatabaseQuery_URIs}
\end{table}
...
Location location = 
  locationManager.getLastKnownLocation(LocationManager.GPS_PROVIDER);
	\end{lstlisting}
	\end{tabular}
	\begin{lstlisting}[caption={Example of a contact database query},
