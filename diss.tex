...
Completeness of the CFG is crutial in order to produce correct liveness intervals of variables and not miss any clashes. A lot of effort therefore had to be invested into reading the (often incomplete) Dalvik documentation to make sure \emph{every} instruction provided a complete list of its successors, reflecting all aspects of the execution semantics, even including the catch blocks the program can jump to if exceptions of various types are thrown. Similarly, instructions throwing exceptions not caught by the surrounding try block must return \verb$true$ when asked whether they can exit the method.
...
\begin{figure}[t]
	\scriptsize
	\begin{verbatim}
I/ActivityManager(  187): Starting: Intent { cmp=com.virsir.android.chinamobile10086/.Root }
                            from pid 11333
I/System.out(11333): $ system service request: alarm => 0
E/FlurryAgent(11333): onEvent called before onStartSession.  Event: ActivityCreate:Root
E/FlurryAgent(11333): onEvent called before onStartSession.  Event: ActivityCreate:MainView
D/FlurryAgent(11333): Starting new session
D/FlurryAgent(11333): Sending report to: http://data.flurry.com/aar.do
I/System.out(11333): $ system service request: wifi => 0
I/System.out(11333): $ content query: content://telephony/carriers/preferapn => 0
I/System.out(11333): $ system service request: phone => 32
I/System.out(11333): $ system service request: wifi => 0
I/System.out(11333): $ system service request: phone => 32
I/System.out(11333): $! tainted Apache HTTPClient request executed => T=32
W/WiYun   (11333): Failed to get ad
W/WiYun   (11333): Failed to get ad
	\end{verbatim}
	\caption{Log output generated by the instrumented version of BgServ.}
	\label{figure:Evaluation_BgServ}
\end{figure}

Unlike GoneSixty which steals personal data, BgServ is interested in information that can identify the device: phone number, IMEI, version of the system, etc. These are accessed through system service managers and from the output in figure~\ref{figure:Evaluation_BgServ} we see that Dexter recognized access to the \texttt{TelephonyManager}\footnote{\scriptsize\url{https://developer.android.com/reference/android/telephony/TelephonyManager.html}} singleton and that the data are again leaked being via HTTP. 

BgServ also acquires the \texttt{WifiManager}, but no taint is assigned since access to network settings is not being tracked. Supporting it is simply a matter of adding the string identifier to a list of recognized services and allocating a flag bit in the taint tag representation. Same applies to the database query requesting network provider information. 

Moreover, the security report mentions that data are written into a temporary file before they are sent to the server. Bytecode inspection revealed that BgServ uses an instance of \verb$XmlSerializer$\footnote{\scriptsize\url{https://developer.android.com/reference/org/xmlpull/v1/XmlSerializer.html}}, which is currently not a recognized sink.

\subsection{Find\&Call}

Find\&Call is a contact organizer, whose iOS version bacame known as the first malware for iOS, approved by Apple and distributed through App Store. Likewise, it briefly appeared on the open Google Play before it was removed in July 2012. While logging into user's online account, Find\&Call leaks contact list and phone number which are used by the server to send spam via text messages that seem to have been sent by the user. A blog entry\footnote{\scriptsize\url{https://www.securelist.com/en/blog/208193641/Find_and_Call_Leak_and_Spam}} by a Kaspersky Lab expert provides more information.

Since the instrumented Find\&Call generates a rather extensive output, the complete log was omitted. Nevertheless, Dexter discovered multiple HTTP requests leaking the contents of the phonebook and user's location. Access to the service providing phone number information was also identified, but tagged data never reached the sink, which might indicate a bug in the implementation, or the fact that they simply were not leaked. Since the application is complex and its bytecode had been obfuscated, further investigation of this problem was not successful. The output also shows that taint automatically propagates between individual application components (activities) through IPC calls.
...
\begin{figure}
	...
	\caption{Time taken to initialize the application and access sources. Error bars represent interval of one standard deviation.}.
...
\begin{figure}
	...
	\caption{Points scored in QuickBenchmark. Error bars represent interval of one standard deviation.}.
...
\begin{figure}
	...
	\caption{Difference between sizes of \emph{classes.dex} files before and after instrumentation, expressed as percentage of the original size.}.
...
\begin{figure}
	...
	\caption{Number of occurances of selected instructions in GoneSixty. First column represents the original count, second the state after application of instrumentation rules, and third the total count after assembling.}.
	\label{figure:Evalutaion_CodeBloat}
\end{figure}

instruction count: orig, instrumented, assembled \\
file size: orig, assembled
memory usage? data in proc

\section{TaintDroid Comparison}
