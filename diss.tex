...
All DEX files are parsed by DexLib, its data structures then traversed and all relevant pieces of information passed to the class hierarchy. Same is done for JAR files, only these are parsed by the BCEL library. Once all of the files have been scanned, \verb$DexClassHierarchy.checkConsistency$ method is called, which makes sure that:
\begin{inparaenum}[(i)]
...
\end{inparaenum}
and creates links between nodes for efficient traversal. At this point, there cannot be any loops in the hierarchy, because every loop-introducing node is rejected during the file-scanning phase.

Class hierarchy provides methods that return pieces of information stored directly in its nodes, or methods that perform simple tree traversal, like returning the set of all ancestors of a class. The destination-deciding algorithm for method calls, which was already discussed in section~\ref{section:TaintPropagation_DestDecision}, is the only non-trivial method inside this class, and hence, there is no point in describing these in greater detail. The only subtlety worth mentioning is that method calls can be performed not only on class types but also on arrays. Since their superclass is always \verb$Object$, i.e. they offer methods like \verb$equals$ and \verb$hashCode$, and they are not allowed to define any new methods, Dexter simply treats them as classes of type \verb$Object$. 

\section{Intermediate Code}

Bytecode for the Dalvik VM, and ultimately the overall structure of the DEX file, was designed so that it can be directly mapped to memory and code executed efficiently. However, this makes direct instrumentation of the file an impossible task, because even the smallest modification breaks a multitude of pointers and memory offsets carefully generated by the JVM-Dalvik conversion tool.

Reasons:
\begin{inparaenum}[(i)]
\item instruction variants
\item numeric offsets
\item numeric registers (constraints)
\item implicit wide registers
\item overlapping try blocks
\end{inparaenum}
