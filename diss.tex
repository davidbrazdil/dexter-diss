...
First distinction is that Dalvik does not distinguish between CPU registers and external memory. Methods can use up to 65k virtual registers stored on the stack, yet most instruction formats only have space to address 4- or 8-bit identifiers. Hence colouring is not limited to a fixed number of colours as each node has its own upper identifier limit given by bit representations of the least optimized variants of instructions that use the corresponding intermediate register.

The way heuristic assigns colours is changed to reflect this difference. As not to pollute smaller ranges, colours are preferentially assigned from the higher parts of the spectrum. So for instance, for node restricted to the \mbox{0-255} range a colour in \mbox{16-255} will be sought first. The spectrum is compacted before result is returned.
