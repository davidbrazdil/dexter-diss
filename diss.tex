...
However, the change in distribution of opcodes is radical. Figure~\ref{figure:Evalutaion_CodeBloat} shows some selected intermediate instruction types and how many times they appeared in the parsed, instrumented and pre-assembly representation of the GoneSixty spyware. We see instructions that are virtually uneffected, like \verb$BinaryOpLiteral$ and \verb$ReturnVoid$, which are only inserted with the linked auxiliary classes, and there are slightly more \emph{if} statements, mostly used for decisions during method calls and source/sink identification. On the other hand, the increase in the number of instruction types directly involved in tainting is extreme, which is especially visible with \verb$invoke$ opcodes jumping from 400 to more than 3000 occurances. These are nearly exclusively \verb$taint-get$ and \verb$taint-set$ calls, which go hand in hand with insertion of \verb$BinaryOp$ \emph{OR} instructions to combine tags. Also note the increase in number of \verb$move$s during the assembly phase as a result of register spilling.
