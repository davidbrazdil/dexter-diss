...
Dexter builds the hierarchy by scanning the application's executable, plus a set of provided framework files. These are all DEX files obtained from the device's \verb$/system/framework$ folder, which contain the classes available at runtime. The framework changes with every new release of Android, and therefore a version compatible with the application must be used, which is the reason why Dexter offers the user to provide their own framework rather than shipping with one particular version. Currently, Dexter also supports scanning JAR files with BCEL to allow SDK packages to be included in the framework, for example for instrumentation of applications dependent on Google Maps API. But since these SDK packages sometimes do not contain the full class hierarchy, this feature might be dropped and the structure obtained from the respective Android application.

The class hierarchy is a simple tree structure, with each node except the root having exactly one parent, and is implemented as such. Each node contains:
\begin{inparaenum}[(i)]
...
Most of the methods offered by the class hierarchy class are trivial and there is no point in describing any of their details, because they only perform a simple tree traversal and collect information. Is it worth mentioning at all?
