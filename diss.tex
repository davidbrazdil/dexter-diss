...
Only part of the file structure that requires more work are the methods and their bytecode. Each instruction is asked to produce the most optimized Dalvik variant it can, being given the register allocation and offsets of other code elements. The size of the generated Dalvik instruction is used to update the offset list. Since changing the offsets might result in some instructions having to produce a different variant, these steps are repeated until no more changes are made. Resulting offsets are then used to generate a list of try blocks and the addresses of catch markers. 

Just like with register identifiers, the space for offsets inside instructions is limited, and thus inserting tainting instructions between these and their targets might result in them not being able to store the offsets. Each such instructions therefore needs to implement a \verb$fixLongJump$ method, in which it generates instrumentation that deals with this. An example is shown in figure~\ref{figure:Reassembling_FixLongJump}. Since \verb$goto$ can store up to 32-bit long offsets, it can reach any point of the bytecode. 
