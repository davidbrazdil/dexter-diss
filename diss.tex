...
Current state of the art in data protection on smartphones lags behind the techniques described above. On Android, developers can freely publish their applications on the official store, and the user is merely shown a list of permissions an installed application requests without any information about the reasons what they might be needed for. Programs can be removed from the store if they are identified as malware either by the users\cite{web:LookoutPlayRemoval} or by an automated system called Bouncer\cite{web:Bouncer}. Situation is slightly better on the rivalling iOS, where each application must pass a manual approval process before being published on the store, the only official way of installing applications, and users grant access to a resource the first time it is used, at least giving the user the context in which data are needed.

However, both of these approaches fail to protect the user against applications that have a legitimate initial reason to read their private data but secretely leak them at the same time, since once access is granted the applications are free to use the resource in any way they want.
