...
Android/GoneSixty is a spyware which was, according to the author, written only for educational purposes. Initially available on the Google Play market, users would download the application, and then watch it send their sensitive data to a remote server immediately after launch. Even though GoneSixty is virtually harmless and its methods of distribution and data collection unsophisticated, it nicely demonstrates access to various sensitive sources, and propagation of the data to a sink. Security report by AVG, with more details on how GoneSixty works, can be found in appendix~\ref{chapter:Report_Gone60}.

\begin{figure}[t]
	\scriptsize
	\begin{verbatim}
I/ActivityManager(  187): Starting: Intent { act=android.intent.action.MAIN 
                                             cat=[android.intent.category.LAUNCHER] 
                                             flg=0x10200000 
                                             cmp=com.gone603/com.gone60.gone60 } from pid 295
I/ActivityManager(  187): Start proc com.gone603 for activity com.gone603/com.gone60.gone60: 
                          pid=10912 uid=10065 gids={3003}
D/dalvikvm(10912): GC_EXTERNAL_ALLOC freed 47K, 51% free 2679K/5379K, external 0K/0K, paused 48ms
D/dalvikvm(10912): GC_EXTERNAL_ALLOC freed 1K, 51% free 2680K/5379K, external 1K/513K, paused 47ms
D/dalvikvm(10912): GC_EXTERNAL_ALLOC freed <1K, 51% free 2680K/5379K, external 601K/1113K, paused 50ms
...
\end{inparaenum}
and store the data into commonly accessible class fields. The fifth thread waits for the other four to finish and then combines the data into a single JSON request, converting them to base64 first, and sends these to a remote server using the Apache HTTP Client. Output of the instrumented application in figure~\ref{figure:Evaluation_GoneSixty} shows that Dexter correctly identified access to all four of these sources, and successfully propagated the taint tags all the way to the HTTP request execution.

\subsection{BgServ}
Analysis in appendix~\ref{chapter:Report_BgServ}.
