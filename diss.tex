...
External call tainting policy also relies on combining taint of all data accessible to the method, even through multiple references. It is possible to recursively traverse all its fields, but this would prove extremely slow for private fields which would require reflection. Instead, instructions assigning to instance fields are instrumented to propagate taint to the containing object, so that it combines the taint of all its fields. Internal fields also have separate taint storage to store their own subset of object taint, but external ones do not because their value could be overwritten by untrackable code which would not update the taint anyway.

This logic also contains a loophole which could lead to unidentified leakage. The problem is that taint does not propagate all the way to the root object in multi-level data structures. Current sinks cannot be exploited this way but serialization can. Apart from structure traversal, one could again use taint unification to solve this issue.
...
In the following code examples, the \verb$TAINT_SOURCE$ and \verb$TAINT_SINK$ constants will be used. These are bit masks that correspond to the regions of tag representation reserved for source and sink flags as defined in Table~\ref{table:TaintTagStorage_BitMeaning}.

\subsubsection{Always-leaking API Calls}

Most tracked API calls are easy to handle, because they always leak the data they get. Sinks implemented this way are listed in table~\ref{table:Sinks_AlwaysLeaking}, and more could easily be added.
...
Listing~\ref{listing:Sink_ApacheHTTPClient} shows a piece of code that sends data to a remote server using the Apache HTTP Client included in the framework. Thanks to taint propagation, \verb$post$ inherits the tag of \verb$data$, which is checked when it is passed to \verb$execute$.
