...
Instead of checking the degree of the node to identify uncolourable graphs, the graph-colouring algorithm always creates an ordering of nodes and tries to find a non-conflicting colour for each one based on the range-limits specified by the intermediate instructions that use the corresponding register object. As not to pollute smaller ranges, colours are first assigned from the higher parts of the spectrum, so for example for node restricted to the 0-255 range a colour in 16-255 will be assigned, and the full 0-255 range will be used only if the former fails. If a solution is found, the mapping is compacted by decreasing register identifiers to remove spaces between used colours.



% Register allocation for Dalvik has its specifics. The most notable one is that it does not differentiate between CPU registers and external memory. Instead, methods are allowed to use up to 65,536 registers, and the VM automatically maps those with smallest identifiers to CPU registers and the rest to RAM. Most instructions are capable of addressing only the first 16 or the first 256 registers, and only the \verb$move$ instructions and the \verb$/range$ variants of \verb$invoke$ and \verb$filled-new-array$ allow to address the rest. 
