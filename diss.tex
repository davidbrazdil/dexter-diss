...
Building a CFG is not complicated. Each element of the executable code, i.e. both instructions and markers, can specify whether it starts or ends a basic block, whether it can exit the method, as well as the complete list of its successors. If it has more than two, it is assumed it ends a block. Code is scanned linearly, instructions put in the current basic block, and new block is created whenever an instruction starting or ending a block is encountered. Blocks are then connected according to the lists of successors of their last instruction. 

Completeness of the CFG is crutial in order to produce correct liveness intervals of variables and not miss any clashes between variables. A lot of effort therefore had to be invested into reading the (often incomplete) Dalvik documentation to make sure each instruction provided a complete list of its successors, reflecting all aspects of its execution semantics, even including the catch blocks program can jump to if it throws an exception. Similarly, instructions throwing exceptions not caught by the surrounding try block must return \verb$true$ when asked whether they can exit the method.

% Register allocation for Dalvik has its specifics. The most notable difference is that it does not provide the notion of external memory. Instead, methods are allowed to use up to 65,536 registers, and the VM automatically maps those with smallest identifiers to CPU registers and the rest to RAM. Most instructions are capable of addressing only the first 16 or the first 256 registers, and only the \verb$move$ instructions and the \verb$/range$ variants of \verb$invoke$ and \verb$filled-new-array$ allow to address the rest. 

% The goal of register allocation is therefore finding an assignment of numerical identifiers to the unconstrained intermediate register objects, which can be thought of as equivalents of programming language variables, such that each instruction can generate one of the semantically equivalent Dalvik instructions. 

% The number of variables used in the source code is typically larger than the number of available CPU registers. Register allocation therefore becomes an optimization problem 


% The algorithm is allowed to assign the same identifier to multiple intermediate registers, provided the intervals between storing values and reading them back do not overlap. Compilers perform a so-called Live Variable Analysis of the source code, building a Clash Graph, to acquire this information. 

% \subsubsection{Dalvik Constraints}

% In order to do this, the assignment needs to satisfy two types of constraints. Depending on the number of bits available for the identifiers inside the representation of the least optimized Dalvik variant, each instruction can specify the numerical range its registers should be allocated to. Some instructions will also need some of their registers to have sequential identifiers. Most commonly, this is required by instructions that operate on adjoining register pairs storing 64-bit values. There are also the \verb$/range$ variant of the \verb$invoke$ instructions, used to call methods with more than five arguments, which addresses consecutive registers by specifying only the first one and their total count. Third constraint of parameter registers being allocated to the highest used identifiers was removed inserting extra move instructions and allocating them manually, as explained in section~\ref{section:Code_ParamRegMapping}. 
