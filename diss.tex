...
\end{inparaenum}
and creates links between nodes for efficient traversal. There also cannot be any loops in the hierarchy, because every loop-introducing node is rejected during the file-scanning phase.
...
Based on the nature of the instrumentation rules introduced earlier, it would be convenient to represent bytecode in a way that no constraints got broken if: 
\begin{inparaenum}[(i)]
...
Similarly, instrumentation will introduce new constants, classes, methods, fields and annotations into the structure of the resulting DEX file.  These can be more or less seamlessly inserted by DexLib, but because 
